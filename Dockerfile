...
FROM ubnt/unms:0.14.1 as unms
FROM ubnt/unms-netflow:0.14.1 as unms-netflow
FROM ubnt/unms-fluentd:0.14.1 as unms-fluentd
FROM ubnt/unms-nginx:0.14.1 as unms-nginx
#FROM oznu/s6-node:10.15.1-debian-amd64
FROM redis:5.0.5-alpine as unms-redis
FROM postgres:9.6.12-alpine as unms-postgres
FROM rabbitmq:3.7.14-alpine	as unms-rabbitmq
	
# container_name: ucrm
# FROM ${UCRM_DOCKER_IMAGE}:${UCRM_DOCKER_TAG}	
...
# ADD https://github.com/Ubiquiti-App/UNMS/blob/master/unms-0.14.1.tar.gz /tmp/unms.tar.gz
